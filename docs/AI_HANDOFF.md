...
- Fixed channel namespaces are generated by both updaters, validated by settings,
  and applied at both nginx proxy routes through the njs header adapter.
- Read COOKIES.md before editing: the unscoped IsAuthenticated cookie is only a
  constant browser hint and is always removed on ingress. It must NEVER become
  an authentication bypass. Display cookies remain shared; credentials do not.
- No upstream pins, URLs, databases or keys change. Existing users sign in once
  again after updating. Do not silently import old unscoped sessions.
- Run the Node cookie tests and two-real-container cookie_smoke.py. Its synthetic
  session fixtures are not real passkey/browser enrollment evidence. Never print
  cookies or raw logs. User acceptance steps are documented separately.
- Keep cookies.mjs byte-identical across packages and namespaces stable across
  delivery updates. Different ports alone cannot isolate cookies.
