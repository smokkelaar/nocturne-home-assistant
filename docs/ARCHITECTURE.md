...
Specific OIDC, OAuth, discovery and hub paths route to API; ordinary paths route to web. From wrapper 0.1.6, guarded native mode forwards only syntactically valid Bearer headers. A v4 request carrying such a header routes directly to the API with its original path/query, so Nocturne performs token, tenant and scope validation. Requests without one retain the web session bridge. Basic mode strips all incoming Authorization and never admits a Bearer token through its extra gate. Dev/operator endpoints and internal instance-auth headers remain blocked. No gateway credential is substituted for an OAuth token. Legacy API-secret clients and real client consent/refresh behavior require separate validation.
