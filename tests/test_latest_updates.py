...
class LatestUpdateTests(unittest.TestCase):
    def setUp(self):
        self.lock = json.loads((ROOT / 'upstream-latest.json').read_text(encoding='utf-8'))
    ...
    def test_update_bumps_only_latest_delivery_and_preserves_wrapper_and_identity(self):
        with tempfile.TemporaryDirectory() as directory:
            root = self.fixture(directory)
            original = json.loads((root / 'nocturne_latest/config.json').read_text(encoding='utf-8'))
            ...
            runtime = json.loads((root / 'nocturne_latest/rootfs/opt/nocturne-ha/version.json').read_text(encoding='utf-8'))
            ...
            self.assertIn(self.candidate()['commit'][:7], (root / 'nocturne_latest/CHANGELOG.md').read_text(encoding='utf-8'))
