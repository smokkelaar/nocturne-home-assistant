...
class PersonalTests(unittest.TestCase):
    ...
    def test_generated_feature_docs_follow_extension_version(self):
        ...
        for path in ('DOCS.md', 'README.md'):
            self.assertIn(b'Personal 0.2.99 previews the cleaned Google Health connector', generated[path])
            self.assertIn(b'temporary Google Health database schema has been removed', generated[path])
