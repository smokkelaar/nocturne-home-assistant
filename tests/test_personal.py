...
class PersonalTests(unittest.TestCase):
    ...
    def test_generated_feature_docs_follow_extension_version(self):
        ...
        for path in ('DOCS.md', 'README.md'):
            self.assertIn(b'Personal 0.2.99 previews only the configurable Year Overview color ranges', generated[path])
            self.assertIn(b'does not include the separate Google Health contribution', generated[path])
