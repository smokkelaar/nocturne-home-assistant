...
def render(root, lock, app_version):
    ...
    return {
        ...
        'nocturne_latest/rootfs/opt/nocturne-ha/version.json': dumps({
            ...
            'nocturne': 'main@' + lock['commit'][:7], 'repository': PROJECT,
            'source_commit': lock['commit'], 'commit_at': lock['commit_at'],
            'base': 'Nocturne main', 'base_commit': lock['commit'],
            'release': 'Geen release; vastgezette main-snapshot',
            'release_url': f"https://github.com/{PROJECT}/tree/{lock['commit']}",
            'purpose': 'De nieuwste succesvol gebouwde main-versie vroeg testen, vóór een officiële release.',
            'purpose_url': f"https://github.com/{PROJECT}/commit/{lock['commit']}",
            'test_plan': 'Containerstart, upgrade vanaf de vorige Latest, basiswerking en regressies van main controleren.',
            'test_url': f"https://github.com/{PROJECT}/actions/runs/{lock['workflow_run']}",
