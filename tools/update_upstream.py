...
def render(root, lock, app_version):
    ...
    return {
        ...
        'nocturne_local/rootfs/opt/nocturne-ha/version.json': dumps({
            ...
            'source_commit': lock['commit'], 'base': f"Nocturne {lock['tag']}",
            'base_commit': lock['commit'], 'release': lock['version'],
            'release_url': f"https://github.com/{PROJECT}/releases/tag/{lock['tag']}",
            'purpose': 'De stabiele, officieel gepubliceerde Nocturne-release gebruiken.',
            'purpose_url': f"https://github.com/{PROJECT}/releases/tag/{lock['tag']}",
            'test_plan': 'Reguliere werking, upgrades en databasemigraties van de officiële release controleren.',
            'test_url': f"https://github.com/{PROJECT}/releases/tag/{lock['tag']}",
            'name': 'Nocturne Official Release',
